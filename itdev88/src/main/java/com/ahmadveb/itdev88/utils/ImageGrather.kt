...
object ImageGrather {
    ...
    fun takeScreenshot(context: Context, nestedScrollView:NestedScrollView) {
        ...
        if (mBitmap != null) {
            ...
            val direct = File("${Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES)}/bookingcar" )
            if (!direct.exists()) {
                val mkdir = direct.mkdir()
            }
            val extr = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES).toString()
            val fileName = "nguberapp/img_" + System.currentTimeMillis().toString() + ".jpg"
            ...
            try {
                ...
                fos!!.close()
                val path =
                    MediaStore.Images.Media.insertImage(context.getContentResolver(), mBitmap, "nguberapp", "nguberapp")

                //share bitmap from uri
                Grather.shareBitmapToApps(context, Uri.parse(path))
                return
    ...
    fun downloadScreenshot(context: Context, nestedScrollView:NestedScrollView) {
        ...
        if (mBitmap != null) {
            ...
            val direct = File("${Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES)}/nguberapp" )
            if (!direct.exists()) {
                val mkdir = direct.mkdir()
            }
            val extr = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES).toString()
